...
public class StudentsService implements IStudentsService{
    private final Logger logger = LogManager.getLogger(StudentsService.class);
            ...
            if (student.getTypeOfStudying() == TypeOfStudying.PRESENT) {
                ...
                if (Objects.nonNull(group)) {
                    if (group.isOnlineAccessible())
                        ...
                        logger.warn(group.getId() + " " + properties.getString("studentsService.not_online_accessible")
                                + properties.getString("studentsService.adding_interrupted"));
                } else
                    logger.warn(student.getGroupId() + " " + properties.getString("studentsService.group_not_exist")
                            + properties.getString("studentsService.adding_interrupted"));
            }
        }
    }
    ...
    private boolean studentIsAddableToGroup(Student student) {
        ...
        if (group != null) {
            ...
            if (numberOfStudentsInGroup < group.getMaxAttendeesPresent())
                ...
                logger.warn(group.getId() + " " + properties.getString("studentsService.group_is_full")
                        + properties.getString("studentsService.cannot_added"));
                return false;
            }

        } else {
            logger.warn(properties.getString("studentsService.not_existing_group")
                    + properties.getString("studentsService.cannot_added"));
            return false;
        }
    }
    ...
    public void delete(Student student){
        if(StudentValidation.isValid(student)) {
            if(studentExists(student.getId()))
                ...
                logger.warn(properties.getString("studentsService.not_exist")
